...
model = "SABR_Experiments/run/models/sabr_cnn_e"
...
def next_batch_sabr_iv(batch_size,only_prices=True):
    ...
    for batch in range(batch_size):  
        for i in range(num_maturities):
            for j in range(num_strikes):
                X[batch,i,j,2] = sabr_iv(y[batch,0],y[batch,1],y[batch,2],X[batch,i,0,1],X[batch,0,j,0],S0,V0)
...
convo_3_flat = tf.reshape(convo_3_pooling,[-1,32*int(np.power(np.maximum(num_maturities,num_strikes),2)/filter_size/filter_size/filter_size)])
...
with tf.Session(config=config) as sess:
    ...
    for iteration in range(num_steps):
        ...
        X_batch,Y_batch = next_batch_sabr_iv(batch_size,only_prices=True)
...
iv_surface_true_opt,thetas_true = next_batch_sabr_iv(N)
...
with tf.Session() as sess:                          
    ...
    theta_pred_scaled = sess.run(outputs,feed_dict={X: iv_surface_true_opt})
...
plt.savefig('rel_errors_dnn_m3_sabr.pdf') 
